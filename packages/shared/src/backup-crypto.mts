...
async function deriveKey(passphrase: unknown, salt: Buffer): Promise<Buffer> {
  const value = String(passphrase ?? '');
  if (value.length < 12) throw new Error('Backup passphrase must contain at least 12 characters');
  return new Promise((resolve, reject) => {
    crypto.scrypt(value, salt, 32, { N: 32768, r: 8, p: 1, maxmem: 64 * 1024 * 1024 }, (error, key) => {
...
export async function encryptBackup(data: Uint8Array, passphrase: unknown): Promise<Buffer> {
...
export async function decryptBackup(data: Uint8Array, passphrase: unknown): Promise<Buffer> {
  const buffer = Buffer.from(data);
