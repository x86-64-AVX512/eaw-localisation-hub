...
import { TRACKED_PATH_PATTERN } from './constants.mts';

const execFileAsync = promisify(execFile);
const lineEndingPreferenceCache = new Map<string, Promise<string>>();

export function clearTrackedLineEndingPreferences() {
  lineEndingPreferenceCache.clear();
}

export function withoutUtf8Bom(text: unknown): string {
  const value = String(text ?? '');
  return value.startsWith('\uFEFF') ? value.slice(1) : value;
}

export function withUtf8Bom(text: unknown): string {
  return `\uFEFF${withoutUtf8Bom(text)}`;
}

export function normaliseLineEndings(text: unknown): string {
  return String(text ?? '').replace(/\r\n?|\n/gu, '\n');
}

function dominantLineEnding(text: unknown): string {
  let crlf = 0;
  let lf = 0;
  let cr = 0;
  const value = String(text ?? '');
...
export function preserveLineEndings(text: unknown, referenceText: unknown, preferredEnding = ''): string {
...
async function preferredTrackedLineEnding(repositoryRoot: string, absolutePath: string,
...
export function applyUtf8ByteEdit(text: string, positionByte: number,
  deleteBytes: number, insertedText: string): string {
...
function retreatFromSplitSurrogate(text: string, index: number): number {
  if (
    index > 0 &&
    index < text.length &&
    /[\uD800-\uDBFF]/.test(text[index - 1]) &&
...
export function computeSingleReplace(previous: string, next: string) {
  if (previous === next) {
    return null;
  }
...
export function normaliseTrackedPath(repositoryRoot: string, absolutePath: string): string {
...
function resolveTrackedPath(repositoryRoot: string, absolutePath: string) {
...
function assertCanonicalContainment(canonicalRoot: string, canonicalFile: string, absolutePath: string): void {
...
export async function readTrackedTextFile(repositoryRoot: string, absolutePath: string): Promise<string> {
...
export async function writeTrackedTextFile(repositoryRoot: string, absolutePath: string, text: string, {
  isCurrent = () => true, expectedText,
}: { isCurrent?: () => boolean | Promise<boolean>; expectedText?: string } = {}): Promise<boolean> {
  const { resolvedRoot, resolvedFile } = resolveTrackedPath(repositoryRoot, absolutePath);
  let handle;
  let temporary;
  let staged;
