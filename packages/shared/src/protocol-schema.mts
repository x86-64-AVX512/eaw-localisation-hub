...
type FieldSpec =
  | { kind: 'string'; maximumBytes: number; required: boolean }
  | { kind: 'integer'; minimum: number; maximum: number; required: boolean };
const text = (maximumBytes: number, required = true): FieldSpec => ({ kind: 'string', maximumBytes, required });
const integer = (maximum = Number.MAX_SAFE_INTEGER, required = true): FieldSpec =>
  ({ kind: 'integer', minimum: 0, maximum, required });
...
const pluginSchemas: Readonly<Record<string, Record<string, FieldSpec>>> = Object.freeze({
  ...
  documentVariantRequest: { path: pathField, authorId: idField, variantEpoch: text(64, false) },
  documentVariantsRequest: { path: pathField },
});

function validateField(message: Record<string, unknown>, name: string, specification: FieldSpec): void {
  ...
  if (typeof value !== 'number' || !Number.isSafeInteger(value)
    || value < specification.minimum || value > specification.maximum) {
...
function serverString(value: unknown, label: string, maximumBytes = 4096,
  { optional = false }: { optional?: boolean } = {}): string {
...
function serverInteger(value: unknown, label: string, maximum = Number.MAX_SAFE_INTEGER): number {
...
function serverArray(value: unknown, label: string, maximumItems: number): unknown[] {
...
function validateDirectoryUser(value: unknown): void {
...
function validatePresence(value: unknown): void {
...
function validateDiscussionMessage(value: unknown): void {
...
function validateReviewItem(value: unknown, kind: string): void {
...
export function validateServerMessage(message: Record<string, unknown>): Record<string, unknown> {
  ...
  if (type === 'synced') {
    ...
    if (message.git !== null && message.git !== undefined) {
      ...
      if (typeof git.checkedAt !== 'number' || !Number.isSafeInteger(git.checkedAt)
        || git.checkedAt < 0) throw new TypeError('Git check date must be an integer');
    }
    return message;
  }
  if (type === 'git-status') {
