...
function supportedRelativePath(value: string): string {
...
function runGit(repository: string, args: string[], maximumBytes = 32 * 1024 * 1024): string {
...
function parseHistory(output: string, currentPath: string): FileHistoryEntry[] {
...
export function listFileHistory(repository: string, relativePath: string, options: FileHistoryOptions = {}) {
...
function showFile(repository: string, revision: string, relativePath: string): string {
  const shown = runGit(repository, ['show', `${revision}:${supportedRelativePath(relativePath)}`], 16 * 1024 * 1024);
  return withoutUtf8Bom(shown);
}

function validatedRevision(repository: string, revision: string, label: string): string {
  ...
  const ancestor = runGitSync(['merge-base', '--is-ancestor', resolved, 'HEAD'], {
    cwd: repository, encoding: 'utf8', windowsHide: true, stdio: 'ignore',
  });
...
export function fileHistoryDiff(
  ...
  relativePath: string,
  fromCommit: string,
  fromHistoricalPath: string = relativePath,
  toCommit: string = 'HEAD',
  toHistoricalPath: string = relativePath,
) {
