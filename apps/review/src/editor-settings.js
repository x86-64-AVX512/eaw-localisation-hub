...
const DEFAULTS = { theme: 'dark', fontFamily: 'Consolas', fontSize: 15, lineHeight: 23 };
const EDITOR_THEMES = {
...
export function createEditorSettings({ monaco, editor, showToast }) {
  for (const [name, definition] of Object.entries(EDITOR_THEMES)) {
    monaco.editor.defineTheme(`eaw-${name}`, definition);
  }
  const theme = document.querySelector('#editor-theme');
  const family = document.querySelector('#editor-font-family');
  const size = document.querySelector('#editor-font-size');
  const lineHeight = document.querySelector('#editor-line-height');
