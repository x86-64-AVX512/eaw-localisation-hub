...
const SEGMENTS = [
  ...
  segment('editing', '2. Обычное редактирование', [
    ...
    'Отмена убирает вашу последнюю редакторскую операцию, а повтор возвращает отменённую. После новой правки отменённая ветка повторов может исчезнуть – это обычное поведение истории редактора.',
  ...
  segment('suggestions', '3. Режим «Правки»', [
    ...
    'Отмена в этом режиме убирает последнюю созданную или изменённую вами правку, а повтор восстанавливает её. Перенос строки разрешён на границе слова; редактор не должен позволять разрезать защищённое слово посередине.',
  ]),
...
export function createHelpPanel({ state, token, showToast }: HelpPanelOptions) {
  ...
  async function saveProgress(segment: TutorialSegment): Promise<boolean> {
    try {
      ...
      if (!response.ok) throw new Error(typeof payload.error === 'string' ? payload.error : `Сервер вернул HTTP ${response.status}.`);
      ...
      showToast(`Прогресс обучения не сохранён: ${error instanceof Error ? error.message : String(error)}`, true);
      return false;
    }
  }
  ...
  async function refreshCacheInfo() {
    ...
    try {
      ...
      cacheInfo.textContent = `Не удалось прочитать кэш: ${error instanceof Error ? error.message : String(error)}`;
    }
  }
  function render() {
    ...
    tutorialNext.textContent = index === SEGMENTS.length - 1 ? 'Завершить' : 'Далее';
  }
  ...
  cacheClear.addEventListener('click', async () => {
    ...
    try {
      ...
      showToast(`Не удалось очистить кэш диффов: ${error instanceof Error ? error.message : String(error)}`, true);
  ...
  function refresh() {
    ...
    if (isNew) versionNotice.textContent = `Доступна новая версия EaW Localisation Hub ${state.serverVersion}. Установлена ${state.version}.`;
