...
import type {
  DocumentVariants, DocumentVariantsOptions, DocumentVariantsPayload, LocalSelection, VariantTexts,
} from './document-variants-types.ts';

export function variantTexts(previous: DocumentVariants | null, payload: DocumentVariantsPayload,
  reviewText: string | null = null): VariantTexts {
...
export function retainedAuthorPreviews(previous: DocumentVariants | null, texts: VariantTexts): Map<string, string> {
...
export function createDocumentVariants({
...
}: DocumentVariantsOptions) {
  const selector = requiredElement<HTMLSelectElement>('#document-view');
  const openButton = requiredElement<HTMLButtonElement>('#personal-file-open');
  const dialog = requiredElement<HTMLDialogElement>('#personal-file-dialog');
  const message = requiredElement<HTMLElement>('#personal-file-message');
  const notice = requiredElement<HTMLElement>('#local-file-notice');
  const selectionsElement = requiredElement<HTMLElement>('#personal-file-selections');
  ...
  let authorEpoch = 0;
  const pendingAuthors = new Map<string, ReturnType<typeof setTimeout>>();

  function clearPendingAuthors(): void {
    for (const timer of pendingAuthors.values()) clearTimeout(timer);
    pendingAuthors.clear();
  }

  function displayLine(value: string | null, emptyLabel: string): string {
  ...
  function renderConflicts(force = false): void {
    ...
    for (const conflict of state.documentVariants?.gitConflicts ?? []) {
      ...
      for (const [choice, title] of [['mine', 'Оставить мою правку'], ['git', 'Взять Git']] as const) {
        ...
        button.addEventListener('click', () => {
          ...
          for (const control of row.querySelectorAll<HTMLButtonElement>('button')) control.disabled = true;
        });
        row.append(button);
      }
  ...
  function textFor(view: string): string {
  ...
  requiredElement<HTMLButtonElement>('#personal-file-mine').addEventListener('click', () => {
    send({ type: 'personalFileMaterialize', path: state.path, mode: 'mine' });
    dialog.close();
  });

  function update(payload: DocumentVariantsPayload): void {
    ...
    for (const option of [...selector.querySelectorAll<HTMLOptionElement>('[data-author]')]) option.remove();
  ...
  function updateAuthor(payload: { variantEpoch: string; authorId: string; textBase64: string }): void {
    if (!state.documentVariants) return;
    if (payload.variantEpoch !== String(authorEpoch)) return;
    const pending = pendingAuthors.get(payload.authorId);
