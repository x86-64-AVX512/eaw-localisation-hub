function visibleChildren(element: HTMLElement): HTMLElement[] {
  return [...element.children].filter((child): child is HTMLElement =>
    child instanceof HTMLElement && !child.hidden && getComputedStyle(child).display !== 'none');
}

function unwrappedWidth(element: HTMLElement): number {
...
export function createAppbarLayout(appbar: HTMLElement) {
  const ticketSwitcher = appbar.querySelector<HTMLElement>('.ticket-switcher');
  const actions = appbar.querySelector<HTMLElement>('.actions');
  if (!ticketSwitcher || !actions) throw new Error('Не найдены элементы верхней панели.');
  const switcherElement = ticketSwitcher;
  ...
  function refresh() {
    ...
    frame = window.requestAnimationFrame(() => {
      ...
      const brandWidth = Math.max(420, unwrappedWidth(switcherElement));
      const actionsWidth = unwrappedWidth(actionsElement);
