...
interface AvatarProfileOptions {
  state: { path: string; avatarBase64?: string | null };
  send: (message: { type: 'avatarDelete' | 'avatarSet'; path: string; avatarBase64?: string }) => void;
  showToast: (message: string, isError: boolean) => void;
}

export function createAvatarProfile({ state, send, showToast }: AvatarProfileOptions) {
