param(
    ...
    [string]$InstallDirectory = (Join-Path $env:LOCALAPPDATA 'EaWLocalisationHub\Client-0.8.7F7'),
    [switch]$DoNotLaunch,
    [switch]$Elevated
)
...
if (Test-Path -LiteralPath $installedDll) {
    $backup = "$installedDll.before-0.8.7F7-" + [DateTime]::Now.ToString('yyyyMMdd-HHmmss') + '.bak'
