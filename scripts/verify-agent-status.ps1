...
if ($metadata.Version -ne '0.8.8F3') { throw "Unexpected version: $($metadata.Version)" }
...
if ((Compare-EawHubDisplayVersion -Installed '0.8.8F2' -Recommended '0.7.0F1') -ne 1) {
    throw 'Version comparison did not detect a newer client fix level.'
}
if ((Compare-EawHubDisplayVersion -Installed '0.8.8F2' -Recommended '0.8.8F2') -ne 0) {
    throw 'Equal versions must compare as equal.'
}
if ($null -ne (Compare-EawHubDisplayVersion -Installed 'broken' -Recommended '0.8.8F2')) {
