...
if ($metadata.Protocol -ne 16) { throw "Unexpected protocol: $($metadata.Protocol)" }
if ((Compare-EawHubDisplayVersion -Installed '0.8.4F2' -Recommended '0.8.7F2') -ne -1) {
    throw 'Version comparison did not detect an update.'
}
if ((Compare-EawHubDisplayVersion -Installed '0.8.7F2' -Recommended '0.7.0F1') -ne 1) {
    throw 'Version comparison did not detect a newer client fix level.'
}
if ((Compare-EawHubDisplayVersion -Installed '0.8.7F2' -Recommended '0.8.7F2') -ne 0) {
    throw 'Equal versions must compare as equal.'
}
if ($null -ne (Compare-EawHubDisplayVersion -Installed 'broken' -Recommended '0.8.7F2')) {
